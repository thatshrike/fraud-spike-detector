...
4. **Adversarial vulnerability — gradual ramping evades both detectors (measured, not just theorized).** Both models are tuned to catch sudden, discrete spikes. To test this, a separate synthetic pattern was built where fraud volume ramps up linearly over 48 hours instead of jumping suddenly, using the same fixed thresholds as elsewhere. Recall dropped sharply for both: z-score fell from ~0.77–0.94 (sudden spikes) to 0.29 (gradual ramp); IsolationForest fell from ~0.95–1.00 to 0.54. This confirms both detectors are substantially weaker against slow-building attacks than against sudden ones. IsolationForest degraded less severely, but this comparison used a single ramp instance (one seed) rather than the multi-seed validation used elsewhere in this project, so the gap between the two detectors here should be treated as a preliminary signal, not a confirmed difference. A real deployment would need a complementary trend-detection method (e.g. cumulative-sum or slope-based detection) to close this gap.
